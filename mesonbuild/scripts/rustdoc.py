...
async def run_and_confirm_success(cmdlist: T.List[str], crate: str):
...
class Rustdoc:
    def __init__(self, build: build.Build, tempdir: str, subprojects: T.Set[str]):
            ...
            if 'compiler' in src_block and src_block['language'] == 'rust':
                ...
                for arg in src_block['parameters']:
                    ...
                    if arg == '--test':
                        is_test = True
                        break
                    elif arg in {'--crate-name', '--emit', '--out-dir'}:
                        prev = arg
                    elif arg != '-g':
                        cmdlist.append(arg)
                ...
                if crate_name:
                    ...
                    cmdlist.append('--check-cfg')
                    cmdlist.append('cfg(docsrs)')
                    cmdlist.append('-o')
                    cmdlist.append('doc')
