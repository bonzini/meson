...
def _raw_mapping_to_attributes(raw: T.Mapping[str, object], cls: T.Type[_DI],
...
class Package:
    ...
    def from_raw(cls, raw: raw.Package) -> Self:
        return _raw_mapping_to_attributes(raw, cls, f'Package entry {raw["name"]}')
...
class Dependency:
    ...
    def from_raw(cls, name: str, raw: raw.DependencyV) -> Dependency:
        ...
        return _raw_mapping_to_attributes(raw, cls, f'Dependency entry {name}',
...
class BuildTarget(T.Generic[_R]):
    ...
    def from_raw(cls, raw: _R) -> Self:
        ...
        return _raw_mapping_to_attributes(raw, cls, f'Binary entry {name}')
...
class Library(BuildTarget['raw.LibTarget']):
    ...
    def from_raw(cls, raw: raw.LibTarget, fallback_name: str) -> Self:  # type: ignore[override]
        ...
        return _raw_mapping_to_attributes(raw, cls, f'Library entry {raw["name"]}')
...
class Binary(BuildTarget['raw.BuildTarget']):
    ...
    doc: bool = True
    path: str = os.path.join('src', 'main.rs')
...
class Manifest:
    ...
    def from_raw(cls, raw: raw.Manifest, path: str = '') -> Self:
        ...
        if raw['package'].get('autolib', True) and 'lib' not in raw and \
                path and os.path.exists(os.path.join(path, 'src/lib.rs')):
            raw['lib'] = {}
        fixed = _raw_mapping_to_attributes(raw, cls, 'Cargo.toml package {raw["package"]["name"]}',
...
class CargoLockPackage:
    ...
    def from_raw(cls, raw: raw.CargoLockPackage) -> CargoLockPackage:
        return _raw_mapping_to_attributes(raw, cls, 'Cargo.lock package')
...
class CargoLock:
    ...
    def from_raw(cls, raw: raw.CargoLock) -> CargoLock:
        return _raw_mapping_to_attributes(raw, cls, 'Cargo.lock',
                                          package=lambda x: [CargoLockPackage.from_raw(i) for i in x])
